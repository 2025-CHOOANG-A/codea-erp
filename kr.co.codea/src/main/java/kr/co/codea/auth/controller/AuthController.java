...
public class AuthController {
	...
	/**
	 * 로그인 요청 처리 - Access Token: HttpOnly 쿠키로 설정 - Refresh Token: 응답 본문에 포함
	 */
	@PostMapping("/login")
	public ResponseEntity<TokenDto> login(@RequestBody LoginRequestDto loginRequestDto, HttpServletResponse response) {
		...
		ResponseCookie accessTokenCookie = ResponseCookie.from("accessToken", tokenDto.getAccessToken()).httpOnly(true)
				// .secure(true) // 운영 환경에서 필수
				// .sameSite("Strict")
				.secure(false).sameSite("Lax").path("/")
				.maxAge(Duration.ofMillis(jwtProvider.getAccessTokenExpirationMillis())).build();
	...
	/**
	 * 로그아웃 처리 - DB에서 Refresh Token 제거 - Access Token 쿠키 삭제
	 */
	@PostMapping("/logout")
	public ResponseEntity<Void> logout(HttpServletRequest request, HttpServletResponse httpResponse,
		...
		ResponseCookie deleteCookie = ResponseCookie.from("accessToken", "").httpOnly(true)
				// .secure(true) // 운영 환경에서 필수
				// .sameSite("Strict")
	...
	/**
	 * Access Token 재발급 요청 처리 - 요청 본문에 포함된 Refresh Token 기반으로 재발급 - 새 Access Token은
	 * HttpOnly 쿠키로 설정
	 */
	@PostMapping("/reissue")
	public ResponseEntity<TokenDto> reissue(@RequestBody TokenDto requestTokenDto, HttpServletResponse response) {
		...
		ResponseCookie newAccessTokenCookie = ResponseCookie.from("accessToken", newTokens.getAccessToken())
				.httpOnly(true)// .secure(true) // 운영 환경에서 필수
				// .sameSite("Strict")
				.secure(false).sameSite("Lax").path("/")
				.maxAge(Duration.ofMillis(jwtProvider.getAccessTokenExpirationMillis())).build();
